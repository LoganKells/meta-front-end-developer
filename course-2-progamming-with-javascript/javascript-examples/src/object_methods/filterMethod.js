...
let validatedPhoneNumbers = phoneNumbers.filter(
    num => num.length === 10
)
console.log(validatedPhoneNumbers)

console.log("Validated phone number with area code")
validatedPhoneNumbers = validatedPhoneNumbers.map(
    num => `+44 ${num}`
);
console.log(validatedPhoneNumbers)
...
kmWalked.forEach(
    value => kmWalkedFormated1.push(value.toFixed(2))
);
console.log(kmWalkedFormated1);

console.log("2.2 ------)")
const kmWalkedFormated2 = kmWalked.map(
    value => value.toFixed(2)
);
console.log(kmWalkedFormated2)

console.log("2.3 -------")
const kmWalkedFormated3 = kmWalked.filter(
        value => (value < 40 && value > 3)
)
