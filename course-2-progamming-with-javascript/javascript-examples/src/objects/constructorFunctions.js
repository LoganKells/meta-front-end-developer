...
function Furniture(type, price, deliveryCharge) {
    ...
    this.summarize = function () {
        console.log(
            `${this.type} costs ${this.price} with delivery cost of ${this.deliveryCharge}`
        )
    }
}
...
function Attraction(name, price, description) {
    ...
    this.summarize = function () {
        console.log(
            `${this.name} is ${this.description}. The cost is $${this.price}.`
        )
    }
}

let disney = new Attraction(
    'Disney',
    60,
    'a world famous theme park with hundreds of rides and activities'
)
disney.summarize()
